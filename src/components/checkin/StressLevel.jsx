 ...
 * 스트레스 슬라이더 (단일 축)
 */
function StressSlider({ type, value: sliderValue, icon: SliderIcon, label, onChangeValue }) {
  return (
    <div className="mb-4 last:mb-0">
      <div className="flex items-center gap-2 mb-2">
        <SliderIcon size={16} className="text-text-muted" />
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((num) => (
          <button
            key={num}
            onClick={() => onChangeValue(type, num)}
...
export default function StressLevel({ value, onChange }) {
  ...
  return (
    <div className="p-4 bg-bg-secondary border border-border-default rounded-xl">
      ...
      <StressSlider type="work" value={work} icon={Briefcase} label="업무/학업" onChangeValue={handleChange} />
      <StressSlider type="relation" value={relation} icon={Users} label="대인관계" onChangeValue={handleChange} />
    </div>
  );
}
